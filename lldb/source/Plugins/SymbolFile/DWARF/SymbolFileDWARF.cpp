...
void SymbolFileDWARF::GetTypes(MainDWARFCompileUnit *main_unit, const DWARFDIE &die,
  ...
  if (die) {
    ...
    for (DWARFDIE child_die = die.GetFirstChild(); child_die.IsValid();
      ...
      GetTypes(main_unit, child_die, min_die_offset, max_die_offset, type_mask,
               type_set);
    }
  }
}
...
Function *SymbolFileDWARF::ParseFunction(CompileUnit &comp_unit,
  ...
  MainDWARFCompileUnit *main_unit = GetMainDWARFCompileUnit(&comp_unit);
...
bool SymbolFileDWARF::ParseImportedModules(
  ...
  std::lock_guard<std::recursive_mutex> guard(GetModuleMutex());
  assert(sc.comp_unit);
  MainDWARFCompileUnit *dwarf_cu = GetMainDWARFCompileUnit(sc.comp_unit);
  if (!dwarf_cu)
    return false;
...
user_id_t SymbolFileDWARF::GetUID(MainDWARFCompileUnit *main_unit,
                                  const DWARFDIE &die) {
  if (!die.MainUnitIsNeeded(main_unit))
    main_unit = nullptr;
  return GetUID(die.GetMainDWARFCompileUnit(main_unit), die.GetDIERef(main_unit));
}

user_id_t SymbolFileDWARF::GetUID(MainDWARFCompileUnit *main_unit, DIERef ref) {
...
//  bool is_dwz_common = is_dwz && &main_unit->GetSymbolFileDWARF() != this;
  ...
  lldbassert(!ref.dwo_num()||*ref.dwo_num()<0x1fffffff);
  lldbassert(!ref.main_cu()||*ref.main_cu()<0x1fffffff);
  user_id_t retval =
         user_id_t(ref.dwo_num() ? *ref.dwo_num() : (ref.main_cu() ? *ref.main_cu() : 0x1fffffff)) << 32 |
         ref.die_offset() |
         user_id_t(!ref.main_cu() ? 0 : ref.dwz_common() == DIERef::CommonDwz) << 61 |
         user_id_t(bool(ref.main_cu())) << 62 |
         (lldb::user_id_t(ref.section() == DIERef::Section::DebugTypes)) << 63;

  // FIXME: Expensive
  MainDWARFCompileUnit *main_unit_check;
  lldbassert(GetDIEUnlocked(retval, &main_unit_check) == dwarfdie_check);
...
SymbolFileDWARF::GetDIEUnlocked(lldb::user_id_t uid,
                                MainDWARFCompileUnit **main_unit_return) {
...
Type *SymbolFileDWARF::ResolveTypeUID(MainDWARFCompileUnit *main_unit,
...
Type *SymbolFileDWARF::ResolveTypeUID(MainDWARFCompileUnit *main_unit,
  ...
  if (die) {
    ...
    return ResolveType(main_unit, die);
  }
  return nullptr;
}
...
bool SymbolFileDWARF::CompleteType(CompilerType &compiler_type) {
  ...
  if (dwarf_die) {
    ...
    if (DWARFASTParser *dwarf_ast = GetDWARFParser(*dwarf_die.GetMainDWARFUnit(main_unit)))
      return dwarf_ast->CompleteTypeFromDWARF(dwarf_die, type, compiler_type);
  }
  return false;
}

Type *SymbolFileDWARF::ResolveType(MainDWARFCompileUnit *main_unit,
                                   const DWARFDIE &die,
                                   bool assert_not_being_parsed,
...
bool SymbolFileDWARF::GetFunction(MainDWARFCompileUnit *main_unit,
...
SymbolFileDWARF::GetDIE(const DIERef &die_ref) {
  if (die_ref.dwo_num() && !GetIsDwz()) {
    ...
    SymbolFileDWARF *dwarf = *die_ref.dwo_num() == 0x1ffffffe
                                 ? m_dwp_symfile.get()
                                 : this->DebugInfo()
...
llvm::Optional<uint64_t> SymbolFileDWARF::GetDWOId() {
  if (GetNumCompileUnits() == 1) {
    if (auto comp_unit = GetCompileUnitAtIndex(0))
      if (MainDWARFCompileUnit *cu = GetMainDWARFCompileUnit(comp_unit.get()))
...
void SymbolFileDWARF::ResolveFunctionAndBlock(lldb::addr_t file_vm_addr,
                                              ...
                                              SymbolContext &sc) {
  assert(sc.comp_unit);
  MainDWARFCompileUnit &cu =
      GetMainDWARFCompileUnit(sc.comp_unit)->GetNonSkeletonUnit();
  DWARFDIE function_die = cu.LookupAddress(file_vm_addr);
  DWARFDIE block_die;
  if (function_die) {
    sc.function =
        sc.comp_unit->FindFunctionByUID(function_die.GetID(&cu)).get();
    if (sc.function == nullptr)
...
void SymbolFileDWARF::FindGlobalVariables(
  ...
  m_index->GetGlobalVariables(
      ConstString(basename), [&](MainDWARFCompileUnit *main_unit, DWARFDIE die) {
        if (!sc.module_sp)
...
void SymbolFileDWARF::FindGlobalVariables(const RegularExpression &regex,
  ...
  SymbolContext sc;
  m_index->GetGlobalVariables(
      regex, [&](MainDWARFCompileUnit *main_unit, DWARFDIE die) {
        if (!sc.module_sp)
          sc.module_sp = m_objfile_sp->GetModule();
        assert(sc.module_sp);
...
bool SymbolFileDWARF::ResolveFunction(MainDWARFCompileUnit *main_unit,
  ...
  if (GetFunction(main_unit, die, sc)) {
    Address addr;
...
void SymbolFileDWARF::FindTypes(
  ...
  m_index->GetTypes(name, [&](MainDWARFCompileUnit *main_unit, DWARFDIE die) {
    ...
    if (Type *matching_type = ResolveType(main_unit, die, true, true)) {
      // We found a type pointer, now find the shared pointer form our type
      // list.
      types.InsertUnique(matching_type->shared_from_this());
    }
...
SymbolFileDWARF::FindNamespace(ConstString name,
  ...
  m_index->GetNamespaces(name, [&](MainDWARFCompileUnit *main_unit, DWARFDIE die) {
    if (!DIEInDeclContext(parent_decl_ctx, main_unit, die))
      return true; // The containing decl contexts don't match
    ...
    namespace_decl_ctx =
        dwarf_ast->GetDeclContextForUIDFromDWARF(main_unit, die);
...
TypeSP SymbolFileDWARF::GetTypeForDIE(MainDWARFCompileUnit *main_unit,
                                      const DWARFDIE &die,
                                      bool resolve_function_context) {
  TypeSP type_sp;
  if (die) {
    ...
    if (type_ptr == nullptr) {
      ...
      if (resolve_function_context && parent_die != nullptr &&
          !GetFunction(main_unit, DWARFDIE(die.GetCU(), parent_die), sc))
        sc = sc_backup;
...
TypeSP SymbolFileDWARF::FindCompleteObjCDefinitionTypeForDIE(
  ...
  m_index->GetCompleteObjCClass(
      ...
      [&](MainDWARFCompileUnit *main_unit, DWARFDIE type_die) {
        ...
        Type *resolved_type = ResolveType(main_unit, type_die, false, true);
...
TypeSP SymbolFileDWARF::FindDefinitionTypeForDWARFDeclContext(
  ...
  if (dwarf_decl_ctx_count > 0) {
    ...
    if (type_name) {
      ...
      m_index->GetTypes(dwarf_decl_ctx, [&](MainDWARFCompileUnit *main_unit,
                                            DWARFDIE type_die) {
        // Make sure type_die's langauge matches the type system we are
        // looking for. We don't want to find a "Foo" type from Java if we
        // are looking for a "Foo" type for C, C++, ObjC, or ObjC++.
        if (type_system &&
            !type_system->SupportsLanguage(GetLanguage(*type_die.GetMainDWARFUnit(main_unit))))
          return true;
        bool try_resolving_type = false;
        ...
        DWARFDeclContext type_dwarf_decl_ctx = GetDWARFDeclContext(main_unit, type_die);
        ...
        Type *resolved_type = ResolveType(main_unit, type_die, false);
        if (!resolved_type || resolved_type == DIE_IS_BEING_PARSED)
          return true;
...
VariableSP SymbolFileDWARF::ParseVariableDIE(const SymbolContext &sc,
  ...
  if (!mangled) {
    ...
    if ((parent_tag == DW_TAG_compile_unit ||
         parent_tag == DW_TAG_partial_unit) &&
        Language::LanguageIsCPlusPlus(GetLanguage(*die.GetMainDWARFUnit(main_unit))))
      mangled =
  ...
  if (symbol_context_scope) {
    ...
    var_sp = std::make_shared<Variable>(
        die.GetID(main_unit), name, mangled, type_sp, scope, symbol_context_scope,
              ...
              if (block == nullptr) {
                ...
                if (concrete_block_die)
                  block = sc.function->GetBlock(true).FindBlockByID(
                      concrete_block_die.GetID(main_unit));
              }
  ...
  llvm::call_once(m_dwp_symfile_once_flag, [this]() {
    ...
    if (FileSystem::Instance().Exists(dwp_filespec)) {
      ...
      if (!dwp_obj_file)
        return;
      m_dwp_symfile =
          std::make_shared<SymbolFileDWARFDwo>(*this, dwp_obj_file, 0x1ffffffe);
    }
  });
  return m_dwp_symfile;
}

llvm::Expected<TypeSystem &> SymbolFileDWARF::GetTypeSystem(MainDWARFUnit &unit) {
  return unit.GetSymbolFileDWARF().GetTypeSystemForLanguage(GetLanguage(unit));
}

DWARFASTParser *SymbolFileDWARF::GetDWARFParser(MainDWARFUnit &unit) {
...
CompilerDecl SymbolFileDWARF::GetDecl(MainDWARFCompileUnit *main_unit,
                                      const DWARFDIE &die) {
  if (DWARFASTParser *dwarf_ast = GetDWARFParser(*die.GetMainDWARFUnit(main_unit)))
...
CompilerDeclContext SymbolFileDWARF::GetDeclContext(MainDWARFCompileUnit *main_unit,
                                                    const DWARFDIE &die) {
  if (DWARFASTParser *dwarf_ast = GetDWARFParser(*die.GetMainDWARFUnit(main_unit)))
...
CompilerDeclContext
SymbolFileDWARF::GetContainingDeclContext(MainDWARFCompileUnit *main_unit,
                                          const DWARFDIE &die) {
  if (DWARFASTParser *dwarf_ast = GetDWARFParser(*die.GetMainDWARFUnit(main_unit)))
...
DWARFDeclContext SymbolFileDWARF::GetDWARFDeclContext(MainDWARFCompileUnit *main_unit, const DWARFDIE &die) {
  if (!die.IsValid())
    return {};
  DWARFDeclContext dwarf_decl_ctx =
      die.GetDIE()->GetDWARFDeclContext(die.GetCU()); // DIE.GetCU(), not main_unit.
  dwarf_decl_ctx.SetLanguage(GetLanguage(*die.GetMainDWARFUnit(main_unit)));
...
LanguageType SymbolFileDWARF::GetLanguage(MainDWARFUnit &unit) {
