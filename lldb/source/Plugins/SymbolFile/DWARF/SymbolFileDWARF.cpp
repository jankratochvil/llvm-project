...

{
  ...
  if (comp_unit) {
    MainDWARFCompileUnit *dwarf_cu = GetDWARFCompileUnit(comp_unit);
    if (!dwarf_cu)
      return;
...
Function *SymbolFileDWARF::ParseFunction(CompileUnit &comp_unit,
  ...
  MainDWARFCompileUnit *main_unit = GetDWARFCompileUnit(&comp_unit);
...
bool SymbolFileDWARF::ParseLineTable(CompileUnit &comp_unit) {
  ...
  const dw_offset_t cu_line_offset = dwarf_cu_die.GetAttributeValueAsUnsigned(dwarf_cu,
    ...
    if (num_attributes > 0) {
      ...
      if (!mangled) {
        ...
        if ((parent_tag == DW_TAG_compile_unit ||
          ...
          mangled = GetDWARFDeclContext(main_unit, die)
      ...
      if (symbol_context_scope) {
        SymbolFileTypeSP type_sp(new SymbolFileType(
        ...
        var_sp = std::make_shared<Variable>(
            ...
            symbol_context_scope, scope_ranges, &decl, location, is_external,
            is_artificial, is_static_member);
