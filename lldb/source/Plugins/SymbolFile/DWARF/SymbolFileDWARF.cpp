...

{
  ...
  if (comp_unit) {
    dwarf_cu = GetMainDWARFCompileUnit(comp_unit);
    if (!dwarf_cu)
      return;
...
bool SymbolFileDWARF::ParseImportedModules(
  ...
  std::lock_guard<std::recursive_mutex> guard(GetModuleMutex());
  assert(sc.comp_unit);
  DWARFUnit *dwarf_cu = GetMainDWARFCompileUnit(sc.comp_unit);
  if (!dwarf_cu)
    return false;
...
void SymbolFileDWARF::ResolveFunctionAndBlock(lldb::addr_t file_vm_addr,
                                              ...
                                              SymbolContext &sc) {
  assert(sc.comp_unit);
  DWARFUnit &cu = GetMainDWARFCompileUnit(sc.comp_unit)->GetNonSkeletonUnit();
