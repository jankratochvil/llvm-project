...
Type::Type(lldb::user_id_t uid, SymbolFile *symbol_file, ConstString name,
  ...
  if (symbol_file && symbol_file->GetObjectFile()->GetFileSpec().GetPath().find(".debug.dwz")!=std::string::npos) {
    fprintf(stderr,"%s:%s\n",symbol_file->GetObjectFile()->GetFileSpec().GetPath().c_str(),name.AsCString());
    lldbassert(0);
  }
  if (byte_size) {
